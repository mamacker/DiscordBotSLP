...
sudo chown -R $name:$name $(pwd)

# Replace the fake path in cronjobs by your actual path
echo "Updating files with your path"
sed -i 's/\/home\/USER\/DiscordBotSLP\/DiscordBotSLP.py/'$(echo $(pwd) | sed 's/\//\\\//g')'\/DiscordBotSLP.py/g' cron.py
sed -i 's/\/home\/USER\/DiscordBotSLP\/DiscordBotSLP.py/'$(echo $(pwd) | sed 's/\//\\\//g')'\/DiscordBotSLP.py/g' cron.sh

# Start the cronjob
echo "Starting the cron job"
sh cron.sh

if [[ $(ps -ef | grep -v grep | grep DiscordBotSLP.py) != *"DiscordBotSLP.py"* ]]
then
	printf "Bot is not running.\nTry to check up the crontab with : crontab -l\nElse you can start the bot manually with : python3 DiscordBotSLP.py"